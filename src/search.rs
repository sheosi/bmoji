use emoji::{Emoji, lookup_by_glyph::iter_emoji};
use tantivy::{schema::{Schema, TEXT, Field, STORED}, Index, doc, query::QueryParser, Score, DocAddress, collector::TopDocs, directory::MmapDirectory, Searcher};

pub trait SearchEngine {
    fn search_emojis(&self, emoji: &str, max_count: u16) -> Vec<&'static Emoji>;
}
...
impl DefSearch {
    pub fn new(lang: String) -> Self {DefSearch{lang}}
}

impl SearchEngine for DefSearch {
    fn search_emojis(&self, emoji: &str, _max_count: u16) -> Vec<&'static Emoji> {
    ...
    pub fn new(langs: &[&str]) -> Self {
        ...
        fn has_index(path: &str) -> bool {
            ...
            match MmapDirectory::open(path) {
                ...
                Err(OpenDirectoryError::DoesNotExist(_)) => {std::fs::create_dir_all(INDEX_PATH).unwrap(); false}
        ...
        Self {searcher, glyph, query_parser}
    }
}
...
impl SearchEngine for TantivySearch {
    fn search_emojis(&self, emoji: &str, max_count: u16) -> Vec<&'static Emoji> {
        ...
        let top_docs: Vec<(Score, DocAddress)> =
            self.searcher.search(&query, &TopDocs::with_limit(max_count.into())).unwrap();

        top_docs.into_iter().map(|(_, doc_address)|{
            let retrieved_doc = self.searcher.doc(doc_address).unwrap();
            let a = retrieved_doc.get_first(self.glyph).unwrap().as_text().unwrap();
            lookup(a).unwrap()   
        }).collect()
    }
}
